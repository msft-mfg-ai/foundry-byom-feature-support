...
AGENT_NAME = "byom-tool-web-iq"
GATEWAY_KIND = "static"
WEB_IQ_MCP_URL = os.environ.get("WEB_IQ_MCP_URL")

def main() -> int:
    if not WEB_IQ_MCP_URL:
        print("::warning::Missing env for tool-web-iq: WEB_IQ_MCP_URL")
        return 0
    cfg, project, aoai = build_clients()
    model = gateway_model(MODEL, cfg, kind=GATEWAY_KIND)
    print(f"::group::tool-web-iq model={model}")

    from _shared import make_mcp_tool
    tools = [make_mcp_tool(WEB_IQ_MCP_URL, "web-iq", auth="AgenticIdentity")]
    ...
    conversation = aoai.conversations.create(
        items=[{"type": "message", "role": "user", "content": 'Ask Web IQ for one current headline.'}],
    )
