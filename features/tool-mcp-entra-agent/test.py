"""BYOM test: tool-mcp-entra-agent"""
import os
import sys
...
AGENT_NAME = "byom-tool-mcp-entra-agent"
GATEWAY_KIND = "static"
MCP_SERVER_URL_AGENT_IDENTITY = os.environ.get("MCP_SERVER_URL_AGENT_IDENTITY")

def main() -> int:
    if not MCP_SERVER_URL_AGENT_IDENTITY:
        print("::warning::Missing env for tool-mcp-entra-agent: MCP_SERVER_URL_AGENT_IDENTITY")
        return 0
    cfg, project, aoai = build_clients()
    model = gateway_model(MODEL, cfg, kind=GATEWAY_KIND)
    print(f"::group::tool-mcp-entra-agent model={model}")

    from _shared import make_mcp_tool
    tools = [make_mcp_tool(MCP_SERVER_URL_AGENT_IDENTITY, "byom-mcp-agent", auth="AgenticIdentity")]
