...
- The target environment is **privately networked**, so `ubuntu-latest` GitHub-hosted runners cannot reach the Foundry / APIM private endpoints. The reusable workflow accepts a `runs-on` input — set it to a self-hosted runner label with VNet line-of-sight when wiring up CI for real.
