...
2. `feature.json` fields are typed in `site/src/data/features.ts` — `support_status` ∈ `supported | partial | not_supported | not_confirmed`, `implementation_status` ∈ `ga | preview | in_progress | not_confirmed | tbd`. Use `tbd` whenever `support_status` is `not_supported` (it's misleading to label a maturity for an integration that doesn't work). The site will fail to type-check if you invent new values. Omit `test_file` for a status-only card.
